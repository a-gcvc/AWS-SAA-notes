...
Role that's configured to perform the replication ins't by default trusted by the destination account because it's a seperate AWS account.
...
- **Ownership**: defaultt is that they will be owned by the same account as the source bucket.
