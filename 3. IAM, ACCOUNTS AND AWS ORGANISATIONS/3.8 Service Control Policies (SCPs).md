...
- Service control policies can be attached to the organizaion as a whole by attaching them to the root container, or they can be attached to one or more organizational units.
...
- Even if management account has service policies attached, either directly via an organizational unit, or on the root conatiner of the organization itself, then the management account is never affected by service control polices.
...
This policy means that in the defult implementation, service control policies have no effect since nothing is restricted.
